...
# インプットデータ（1行のデータフレーム）
# 講義のではエラーのため修正：DataFrame オブジェクトに append 属性がないというエラー。
# Pandas のバージョンによっては append メソッドがサポートされていない可能性があり。
# Pandas の新しいバージョンでは、append メソッドが非推奨になり、代わりに concat 関数を使用することが推奨
